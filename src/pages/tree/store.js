...
export default class TreeStore {
  ...
  removeComponent = (identity, tree = this.tree) => {
    ...
    tree.children = tree.children.map((component) =>
  ...
  addNewProp = (prop, identity, data) => {
    if (prop.identity === identity) {
      ...
      if (prop.children) {
        prop.children = [...prop.children, child]
      }
      if (Array.isArray(prop.value)) {
        prop.value = [...prop.value, child]
      }
      return prop
    }
    if (prop.children) {
      prop.children = prop.children.map((item) =>
        this.addNewProp(item, identity, data)
      )
    }
    if (Array.isArray(prop.value)) {
      prop.value = prop.value.map((item) =>
        this.addNewProp(item, identity, data)
      )
    }
    return prop
  ...
  changePropName = (prop, name, identity) => {
    ...
    if (prop.children) {
      // object
      prop.children = prop.children.map((item) =>
        this.changePropName(item, name, identity)
      )
    }
    if (prop.value) {
      // array
  ...
  changePropValue = (prop, v, identity) => {
    ...
    if (prop.children) {
      // object
      prop.children = prop.children.map((item) =>
        this.changePropValue(item, v, identity)
      )
    }
    if (prop.value) {
      // array
  ...
  changePropType = (prop, type, identity) => {
    ...
    if (prop.children) {
      // object
      prop.children = prop.children.map((item) =>
        this.changePropType(item, type)
      )
    }
    if (prop.value) {
      // array
      if (Array.isArray(prop.value)) {
        prop.value = prop.value.map((item) => this.changePropType(item, type))
      }
    }
    return prop
  }
  ...
  changePropFile = (prop, data, identity) => {
    ...
    if (prop.children) {
      // object
      prop.children = prop.children.map((item) =>
        this.changePropFile(item, data, identity)
      )
    }
    if (prop.value) {
      // array
